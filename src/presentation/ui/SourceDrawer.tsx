...
export function SourceDrawer() {
  ...
  const fallback =
    index <= 15 ? `/sources/slide-${String(index + 1).padStart(2, "0")}.png` : undefined;
      ...
      >
        ...
        {sources.length === 0 && !fallback && (
          <p className="text-base text-muted">
            No excerpt is attached to this node. The Layer 1 wording is the claim.
          </p>
        )}
        <div className="space-y-8">
          {sources.map((s) => (
            <article
              ...
              className="rounded-2xl border border-border bg-raised p-5"
            >
              <p className="font-display text-lg text-fg">{s.title}</p>
              {s.section && (
                <p className="mt-1 font-mono text-xs tracking-wide text-gold">{s.section}</p>
              )}
              <blockquote className="mt-4 text-base leading-relaxed text-fg/90 italic">
                “{s.excerpt}”
              </blockquote>
          ...
          {fallback && sources.every((s) => s.artifact !== fallback) && (
            <article className="rounded-2xl border border-border bg-raised p-5">
              ...
              <p className="mt-1 text-sm text-muted">
                Retained as provenance. The interactive stage is the reconstruction.
              </p>
              <img
                src={fallback}
